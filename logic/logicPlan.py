...
def sentence3() -> Expr:
    ...
    exp1_1 = conjoin(PacmanAlive_0, ~PacmanKilled_0)
    exp1_2 = conjoin(~PacmanAlive_0, PacmanBorn_0)

    exp1 = PacmanAlive_1 % disjoin(exp1_1, exp1_2)
    exp2 = ~conjoin(PacmanAlive_0, PacmanBorn_0)
    exp3 = PacmanBorn_0
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    if (findModel(~conclusion & premise)):
        return False
    else:
        return True
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for combo in combos:
        result = disjoin(~combo[0], ~combo[1])
        statements.append(result) 
    
    return conjoin(statements)
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    least_one = atLeastOne(literals)
    most_one = atMostOne(literals)

    return conjoin(least_one, most_one)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    if t > 0 and not(successorAxioms == None):
        pacphysics_sentences.append(successorAxioms(t, walls_grid, non_outer_wall_coords))

    if not(sensorModel == None):
    ...
    for x, y in non_outer_wall_coords:
        outer_wall_pos.append(PropSymbolExpr(pacman_str, x, y, time=t))

    for action in DIRECTIONS:
        actions.append(PropSymbolExpr(action, time=t))
    
    pacphysics_sentences.append(exactlyOne(outer_wall_pos))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    exp1 = conjoin(KB)
    exp2 = PropSymbolExpr(pacman_str, x1, y1, time=1)

    req1 = conjoin(exp1, exp2)
    req2 = conjoin(exp1, ~exp2)
...
def localization(problem, agent) -> Generator:
    ...
    for (coord_x, coord_y) in walls_list:
        KB.append(PropSymbolExpr(wall_str, coord_x, coord_y))

    for (coord_x, coord_y) in all_coords:
        if (coord_x, coord_y) not in walls_list:
            KB.append(~PropSymbolExpr(wall_str, coord_x, coord_y))

    for time_step in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(time_step, all_coords, non_outer_wall_coords, walls_grid, sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[time_step], time=time_step))
        KB.append(fourBitPerceptRules(time_step, agent.getPercepts()))
        possible_locations = []

        for (coord_x, coord_y) in non_outer_wall_coords:
            pacman_here = PropSymbolExpr(pacman_str, coord_x, coord_y, time=time_step)
            if entails(conjoin(KB), pacman_here) and not entails(conjoin(KB), ~pacman_here):
                KB.append(pacman_here)
                possible_locations.append((coord_x, coord_y))
            elif not entails(conjoin(KB), pacman_here) and entails(conjoin(KB), ~pacman_here):
                KB.append(~pacman_here)
            else:
                possible_locations.append((coord_x, coord_y))

        agent.moveToNextState(agent.actions[time_step])
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for coord in non_outer_wall_coords:
            wall_exists = PropSymbolExpr(wall_str, coord[0], coord[1])
