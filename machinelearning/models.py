...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        while True:
            conv = False
            for inp, res in dataset.iterate_once(1):
                if self.get_prediction(inp) != nn.as_scalar(res):
                    nn.Parameter.update(self.w, inp, nn.as_scalar(res))
                    conv = True
                    break
            if not conv:
                break
...
class RegressionModel(object):
    ...
    def train(self, dataset):
        ...
        while True:
            for inp, res in dataset.iterate_once(10):
