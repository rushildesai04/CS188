...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        sorted_items = new_distribution.items()
        ran_num = random.random()
        init = 0

        for key, prob in sorted(sorted_items):
            if ran_num >= init  and ran_num < init + prob:
                return key
            else:
                init = init + prob
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostInJail or nosyDistNone:
            if ghostInJail and nosyDistNone:
                return 1
            else:
                return 0

        manDist = manhattanDistance(pacmanPosition, ghostPosition)
        obsProb = busters.getObservationProbability(noisyDistance, manDist)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            posDist_items = self.getPositionDistribution(gameState, pos).items()
            for temp_pos, temp_prob in posDist_items:
                beliefs[temp_pos] = beliefs[temp_pos] + (self.beliefs[pos] * temp_prob)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        legalLen = len(self.legalPositions)
        for part in range(self.numParticles):
            self.particles.append(self.legalPositions[part % legalLen])
    ...
    def getBeliefDistribution(self):
        ...
        for part in self.particles:
            beliefs[part] = beliefs[part] + self.numParticles
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for part in self.particles:
            if part not in memory:
                obsProb = self.getObservationProb(observation, pacmanPos, part, jailPos)
                memory[part] = obsProb
            else:
                obsProb = memory[part]

            if part in distribution:
                distribution[part] += obsProb
            else:
                distribution[part] = obsProb
    ...
    def elapseTime(self, gameState):
        ...
        for curPart in self.particles:
            if curPart not in dists:
                newPosDist = self.getPositionDistribution(gameState, curPart)
                dists[curPart] = newPosDist
            
            sampledPosition = dists[curPart].sample()
            parts.append(sampledPosition)
