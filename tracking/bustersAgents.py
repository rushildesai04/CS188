...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for posDist in livingGhostPositionDistributions:
            maxPosition = posDist.argMax()
            maxPos.append(maxPosition)
        ...
        minPos = maxPos[minDistanceIndex]

        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            maxSuc.append(successor)
        
        for pos in maxSuc:
            distance = self.distancer.getDistance(pos, minPos)
            sucDist.append(distance)

        minSucIndex = sucDist.index(min(sucDist))
        minSuc = legal[minSucIndex]
